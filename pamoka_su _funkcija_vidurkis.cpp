...
int main()
{
   ...
   vidP=vidurkis(kiekP,P); //kintamajam priskiriu sitos funkcijos su sitais pasiustais parametrais reiksme
...
{
    int suma (0);
    float vid; //papildomas vidinis kintamasis
        for(int i=0; i<kiek; i++)
    {
        suma = suma +Z[i];
    }
    vid = (float)suma/kiek;
    return vid; //neturi galimybes isokti i programa, tai cia isvedam return vid
}
